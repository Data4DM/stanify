...
data {
    ...
    array[100] real s_obs;  // subscripts: timesteps
...
transformed parameters {
    ...
    array[timesteps] vector[n_odes] ode_result = ode_rk45(ode_func, initial_state, initial_time, integration_times, ss2p_frac4, adj_frac3, adj_frac5, adj_frac1, saveper, s2d_frac6, safety_stock_coverage_frac7, adj_frac2);
...
generated quantities {
    ...
    for (i in 1:timesteps){
        loglik += normal_lpdf(s_obs[i] | s[i], sigma_s);
    }
    loglik += lognormal_lpdf(sigma | 0, 1);
    loglik += lognormal_lpdf(sigma_s | 0, 1);
