...
data{
    ...
    vector[10] exog_demand;
    vector[10] process_noise_normal_driving;
...
parameters{
    real<lower=0> adj_frac2_loc;
    array[R] real<lower=0> adj_frac2;
    real<lower=0> adj_frac3_loc;
    real<lower=0> ss2p_frac4_loc;
    real<lower=0> adj_frac1_loc;
    array[R] real<lower=0> adj_frac3;
    array[R] real<lower=0> ss2p_frac4;
...
transformed parameters {
    ...
    for (r in 1:R){
        array[N] vector[5] integrated_result = ode_rk45(vensim_ode_func, initial_outcome, initial_time, integration_times, time_saveper, adj_frac3[r], process_noise_scale, ss2p_frac4[r], adj_frac1[r], adj_frac2[r]);
...
model{
    adj_frac2_loc ~ normal(0.125, 0.0125);
    adj_frac2 ~ normal(rep_vector(adj_frac2_loc, R), 0.0125);
    adj_frac3_loc ~ normal(0.5, 0.05);
    ss2p_frac4_loc ~ normal(0.2, 0.020000000000000004);
    adj_frac1_loc ~ normal(0.25, 0.025);
    adj_frac3 ~ normal(rep_vector(adj_frac3_loc, R), 0.05);
    ss2p_frac4 ~ normal(rep_vector(ss2p_frac4_loc, R), 0.020000000000000004);
    adj_frac1 ~ normal(rep_vector(adj_frac1_loc, R), 0.025);
    m_noise_scale ~ lognormal(0.0, 0.01);
    for (r in 1:R)
