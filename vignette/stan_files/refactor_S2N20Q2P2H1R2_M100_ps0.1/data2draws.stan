...
model{
    ...
    adj_frac1 ~ normal(rep_vector(0.25, R), 0.025);
    m_noise_scale~lognormal(log(1),0.1);
    for (r in 1:R) stocked_ping_obs[:, r]~lognormal(log(stocked_ping[:,r]),m_noise_scale);
    for (r in 1:R) stocked_pping_obs[:, r]~lognormal(log(stocked_pping[:,r]),m_noise_scale);
}
...
generated quantities{
    ...
    for (r in 1:R) stocked_ping_obs_post[:, r]=lognormal_rng(log(stocked_ping[:,r]),m_noise_scale);
    for (r in 1:R) stocked_pping_obs_post[:, r]=lognormal_rng(log(stocked_pping[:,r]),m_noise_scale);

    real loglik;
    real loglik_prior;
    real loglik_stocked_ping;
    real loglik_stocked_pping;
    for (r in 1:R) loglik+=normal_lpdf(adj_frac1[r]|adj_frac1[r],0.025);
    for (r in 1:R) loglik_prior+=normal_lpdf(adj_frac1[r]|adj_frac1[r],0.025);
    for (r in 1:R) loglik+=lognormal_lpdf(stocked_ping_obs[:, r]|log(stocked_ping[:,r]),m_noise_scale);
    for (r in 1:R) loglik_stocked_ping+=lognormal_lpdf(stocked_ping_obs[:, r]|log(stocked_ping[:,r]),m_noise_scale);
    for (r in 1:R) loglik+=lognormal_lpdf(stocked_pping_obs[:, r]|log(stocked_pping[:,r]),m_noise_scale);
    for (r in 1:R) loglik_stocked_pping+=lognormal_lpdf(stocked_pping_obs[:, r]|log(stocked_pping[:,r]),m_noise_scale);
}
