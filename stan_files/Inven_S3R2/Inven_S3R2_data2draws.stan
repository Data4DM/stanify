functions{
    #include Inven_S3R2_functions.stan
}
...
parameters{
    ...
    array[R] real<lower=0> wip_adjustment_time;
}
...
transformed parameters {
    ...
    for (r in 1:R){
        array[N] vector[7] integrated_result = ode_rk45(vensim_ode_func, initial_outcome, initial_time, integration_times, wip_adjustment_time[r], process_noise_scale, inventory_adjustment_time, time_step);
...
model{
    ...
    wip_adjustment_time ~ normal(rep_vector(2, R), 0.02);
    for (r in 1:R)
