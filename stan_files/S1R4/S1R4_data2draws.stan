...
transformed parameters {
    ...
    for (r in 1:R){
        array[N] vector[Q] integrated_result = ode_rk45(vensim_ode_func, initial_outcome, initial_time, integration_times, time_step, prey_birth_frac, pred_birth_frac[r], process_noise_scale);
...
generated quantities{
    ...
    array[N] vector[R] prey_obs_post_pred;
    array[N] vector[R] predator_obs_post_pred;
    // Assign generated value to observed vector (matching vector)
    for (r in 1:R){
        prey_obs_post_pred[:, r] = normal_rng(prey[:, r], m_noise_scale);
        predator_obs_post_pred[:, r] = normal_rng(predator[:, r], m_noise_scale);
    }
