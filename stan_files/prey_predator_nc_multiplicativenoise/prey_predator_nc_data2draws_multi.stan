functions{
    #include prey_predator_nc_functions.stan
}
...
transformed parameters {
    ...
    vector[3] integrated_result[n_t] = ode_rk45(vensim_ode_func, initial_outcome, initial_time, times, time_step, beta, delta, process_noise_scale, gamma, alpha);
...
model{
    alpha ~ normal(0.55, 0.055);
    beta ~ normal(0.028, 0.0028);
    delta ~ normal(0.024, 0.0024);
    gamma ~ normal(0.8, 0.08);
    m_noise_scale ~ normal(0.1, 0.001);
    prey_obs ~ normal(prey, m_noise_scale * to_vector(prey));
    predator_obs ~ normal(predator, m_noise_scale * to_vector(predator));
}

generated quantities{
    vector[200] prey_obs_posterior = to_vector(normal_rng(prey, m_noise_scale * to_vector(prey)));
    vector[200] predator_obs_posterior = to_vector(normal_rng(predator, m_noise_scale * to_vector(predator)));

    real loglik;
    loglik += normal_lpdf(prey_obs|prey, m_noise_scale * to_vector(prey));
    loglik += normal_lpdf(predator_obs|predator, m_noise_scale * to_vector(predator));
}
